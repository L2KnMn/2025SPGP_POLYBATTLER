...
- 게임 캐릭터 동그라미, 네모, 세모 등의 간단한 도형 및 빨간색, 파란색, 초록색 등 색으로 캐릭터 표현, 한 번에 최대 6개의 캐릭터가 파티로 전투 가능하도록 함
    - 동그라미 : 원거리 공격 캐릭터, 낮은 공격력, 범위 공격, 낮은 체력
    - 네모 : 근거리 공격 캐릭터, 높은 공격력, 단일 공격, 높은 체력
    - 세모 : 원거리 공격 캐릭터, 높은 공격력, 단일 공격, 낮은 체력
    - 빨간색, 파란색, 초록색 세 가지 색상 간에 시너지 2 / 4 / 6 단위 버프
      - 빨간색 : 아군 빨간 유닛 공격력 상승
      - 파란색 : 모든 적 공격 속도 하락
      - 초록 : 아군 초록 유닛 체력 상승
    - 도형 숫자 별로 버프 2 / 4 단위 버프
      - 동그라미 : 아군 동그라미 공격 속도 증가
      - 세모 : 아군 세모 공격력 증가
      - 네모 : 아군 네모 방어력 및 체력 보너스
  - 밴치 : 5개까지 상점에서 산 캐릭터를 저장해뒀다가 맵으로 옮길 수 있도록 함
  ...
  - 상점 : 게임 진행 중 매 라운드마다 종료 후 상점 이용 가능
    ...
    - 상품 누르면 구매 후 벤치에 구매한 캐릭터 배치 
    - 리롤 기능
    - 구매한 캐릭터를 되팔 수 있음
    - 캐릭터는 3레벨까지 판매
  - 조합 : 같은 색, 도형 캐릭터를 3개가 모이면 자동 레벨업 -> 4레벨까지 표현
    - 레벨에 따른 외형적 변화는 구현하지 않았고 대신 위에 레벨 표기가 올라감
    - 레벨업시 시각 효과 발생함
  - 맵 : 사각형으로 7 * 4 정도 크기로 구현 플레이어는 아래 편에 3 * 4 사이즈 맵에 자기 캐릭터를 배치해서 진형을 만들도록 하고 자동 전투 진행
    - 배치할 수 없는 부분은 가시성을 위해 좀 더 어둡게 필터를 씌워서 구분
    - 배치할 때 배치 예상 되는 부분 표시하는 기능 존재
  ...
  - 에너미 :
    - 똑같이 동그라미 세모 네모 등의 도형 등장
    - 검은색으로 통일하였고, 엘리트 에너미 없이 동그라미, 세모 네모 캐릭터만 존재.
    - 적의 숫자는 16개 등장, 한 번에 최대 4 * 4 숫자가 등장 가능
    - 웨이브 별도 고려 안 함 -> 한 번에 다 등장 
    - 적의 조합 패턴은 라운드 별로 미리 생성해서 json이나 xml로 저장해뒀다가 사용함
    - 적의 조합에 추가로 이전 라운드 클리어 기록을 가져와서 등장하도록 함

  - 게임 정산
    - 게임 오버 혹은 10라운드까지 게임 클리어 -> UI로 결과 보여줌
    - 터치 등으로 확인 하면 덱을 점수와 함께 기록 -> 이 기록된 덱은 후에 다음 게임에 활용 및 점수 랭킹으로 확인
...
~5월 4주차 : 캐릭터 전투 방식, 그리드 기반으로 전환 및 행동~

~6월 1주차 :  버그 수정 및 완성도 향상~
...
진행 상황
- 게임 기능 ( 100% )
  - 전투 ( 100% ) : 기능 모두 구현 완료, 시각적 효과도 아직 부족
  ...
  - UI ( 100% ) : 버튼, 토스트, 텍스트 보드 등 틀은 잡아두었고, 필요한 것을 생성해서 사용 가능. 캐릭터 레벨 표시, 상점 UI 등 필요한 부분 구현 완료. 다만, 시각적으로 매우 단순하게 구현됨
  - 이펙트 ( 100% ) : 피격 효과 구현 완료, 합성시 Level up 효과 등
- 랭킹 기능 ( 20% )
  - 랭킹 표시 기능 : 씬 구현 및 리소스는 찾아뒀지만 클리어 데이터를 저장 데이터 불러와서 파싱 후 표기하는 걸 아직 구현하지 못함. 
...
- UI
  * 5종류를 만들었습니다. UI는 별도의 공통된 interface는 없고 모두 IGameObject를 상속 받습니다. 생성과 관리도 UI 오브젝트가 각자 책임지고 실행합니다. 다만, 게임 씬의 상태 변화를 전달 받기 위해 Ui manager를 참조해서 현재 Scene의 상태를 보고 그릴지 아닐지, 유저 입력 이벤트를 받아서 처리할지 결정합니다.
    ...
    - Synergy Display : 사용자가 구성한 파티 구성에 따른 추가 효과를 보여주는 UI
- Effect
...
- 직접 개발한 것
    ...
    - 게임 이펙트
    - 게임 매니저 및 게임 상태 관리
    - 터치 시스템(게임 내 오브젝트 터치 드래그로 이동)
    - 상점 시스템
    - 전투 시스템
    - 게임 맵 및 그리드 기반 이동 시스템
    - UI 중 TextBoard, 시너지 버프 UI,
    - 게임 오브젝트 삭제
    - 저장 데이터 읽기 및 그 기반으로 오브젝트 생성
    - 게임 플레이 데이터 저장 및 그 기반으로 오브젝트 생성
...
* 아쉬운 것들
    - 하고 싶었지만 못 한 것들
        - 다양한 전투 시너지 버프 구성 => 단순하게 수치 상승 정도로 그쳤습니다
        - 시각적인 전투 효과 => 많이 단순하게 구현했습니다
        - 다양한 사운드 효과 => 배경 및 터치 효과음 정도만 구현했습니다.
        - 랭킹 시스템 => 저장한 클리어 데이터를 바탕으로 랭킹도 메기고 원격 저장소에 보내서 저장하는 기능을 구현하고 싶었으나 못 했습니다.
    - (앱을 스토어에 판다면) 팔기 위해 보충할 것들
        - 리소스, 시각 효과 등 외적인 부분 보강
        - 빈 사운드 채우기
        - 다양한 언어 지원
